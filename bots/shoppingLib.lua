...
local function TryToReloadShoppingLibFromTeamBotBrain()
	--prevents a warning in lobby where teamBotBrain is not loaded yet
	if HoN.GetMatchTime() == HoN.GetRemainingPreMatchTime() then return end
	
	--We need to call the engine, because core.teamBotBrain is not loaded yet
	local teamBotBrain = HoN.GetTeamBotBrain()
	if teamBotBrain then
		local unitSelf = object:GetHeroUnit()
		local nUID = unitSelf:GetUniqueID()
		if teamBotBrain.tShoppingInfo then
			object.core.BotEcho("Reload Shoppinginfo from Teambot")
			return teamBotBrain.tShoppingInfo[nUID]
		end	
	end
end
object.shoppingLib = TryToReloadShoppingLibFromTeamBotBrain() or {}
...
shoppingLib.tDelayedItems = shoppingLib.tDelayedItems or {}
...
shoppingLib.tRequestedItemsQueue = shoppingLib.tRequestedItemsQueue or {}

--Regen Tresholds
shoppingLib.nMaxHealthTreshold = 1000
shoppingLib.nMaxManaTreshold = 350

--Reload compatibility: Synchronize with TeamBotBrain
shoppingLib.nNextTeamBotBrainCheck = 0
...
function shoppingLib.ShopUtility(botBrain)
	...
	if shoppingLib.bWaitForLaneDecision then
		...
		if nRemainingTime then
			if nRemainingTime >= core.teamBotBrain.nInitialBotMove then 
				return nUtility 
			else
...
function shoppingLib:onThinkShopping(tGameVariables)
	...
	if not shoppingLib.bInitOnThink then
		--Reload compatibility in practice matches
		if tGameVariables and tGameVariables.bIsPractice  and not shoppingLib.bSaveTeambotReferenceForReload then
			BotEcho("This is a practice game. Saving teambot-reference for reload purposes.")
			shoppingLib.bSaveTeambotReferenceForReload = true
		end
		
		--Due to a loading bug we have to stall our shopping experience
		local nReaminingTime = HoN.GetRemainingPreMatchTime()
		if nReaminingTime and nReaminingTime < 89000 then
			tinsert(behaviorLib.tBehaviors, behaviorLib.ShopBehavior)
			shoppingLib.bInitOnThink = true
		end
	end
		
	--Reload bots compatibility
	if shoppingLib.bSaveTeambotReferenceForReload then
		local teamBotBrain = HoN.GetTeamBotBrain()
		if teamBotBrain and teamBotBrain ~= core.teamBotBrain then
			...
			if not teamBotBrain.tShoppingInfo[nUID] then
				...
				teamBotBrain.tShoppingInfo[nUID] = shoppingLib
			end
		end
	end
